     ...
     <table id="dataTable" class="table table-striped table-bordered" cellspacing="0" width="100%">
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td><div id="div1" <?php if($dd1 >= date("Y-m-d") ){?> ondrop="drop(event,{{$drv->id}},{{$d1}},{{$m1}},{{$y1}})" <?php }?> ondragover="allowDrop(event)"> @if(!empty($ldid1)&& $divid1==$drv->id) 
                                <?php
                                ...
                                $expoid=explode('.',$ldid1);
                                  if($expoid)
                                  {
                                    foreach($expoid as $exp) 
                                    {
                                     ...
                                     {{$checkstatus[0]->loadnumber}} &nbsp; 
                                      {{$checkstatus[0]->load_title}} &nbsp;  
                         ...
                         @endif</div></td>
                        <td><div id="div1" <?php if($dd2 >= date("Y-m-d") ){?> ondrop="drop(event,{{$drv->id}},{{$d2}},{{$m2}},{{$y2}})" <?php } ?>ondragover="allowDrop(event)"> @if(!empty($ldid2) && $divid2==$drv->id) <?php
                                $expoid=explode('.',$ldid2);
                                  if($expoid)
                                  {
                                    foreach($expoid as $exp) 
                                    {
                                     ...
                                     {{$checkstatus[0]->loadnumber}} &nbsp; {{$checkstatus[0]->load_title}}
...
// Add record
   ...
   $.ajax({
        url: '{!! asset('deljob') !!}',
        type: 'post',
        data: {_token: CSRF_TOKEN,jobid: jobid,loadid: loadid,driverid: driverid,d: d,m: m,y: y},
        success: function(response){
