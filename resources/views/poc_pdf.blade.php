  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Job No <br />  <b> {{$podlink->job_number}} </b></td>
          <td>Account Detail  <br />{{$customerdetails->name}}</td>
...
<div class="leftimg-sect">
    ...
    <td>
        <div class="imgsect-2">
        <img src="{!! asset('/uploads').'/'.$dentreport[0]->screenshot !!}" width="100px" height="100px"> </img>
        </div>
        </td>
...
<ul>
    @foreach ($dentreport as $dentreport)@if(!empty($dentreport->type))<li>{{$dentreport->type}}@endif</li>@endforeach
<br />
